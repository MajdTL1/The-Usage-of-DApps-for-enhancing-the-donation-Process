  ...
  it("check Category", function() {
    return AddressBook.deployed().then(function(instance) {
      ...
      return ABinstance.getcategoryIndex(6);
        ...
        assert.equal(noCate, "", "correct empty category");
   ...
   it("reg Org + check info", function(){
    ...
    .then(function(i){
        ...
        assert.equal(info1, "first added Address1", "correct infos of 0x11");
        assert.notEqual(info1, "", "correct infos of 0x11");
        return ABinstance.changeInfoOfAddress(accounts[1], 1, 0x11, "changed Infos 1", {from: accounts[2]});
    }).then (function (newInfo){
        return ABinstance.getInfoOf(accounts[1], 1, 0x11);
    }).then(function (changedInfo){
        assert.notEqual(changedInfo, "first added Address1", "correct new infos of 0x11");
        assert.equal(changedInfo, "changed Infos 1", "correct new infos of 0x11");
    });
   });
   ...
   it("change sec Address", function(){
        ...
        .then (function (newInfo){
            ...
            assert(error.message.indexOf('revert') >= 0, "cannot change the Infos");
            return ABinstance.getInfoOf(accounts[1], 1, 0x11);
        }).then(function (changedInfo){
    ...
    it("remove Address 0x11",function(){
        return ABinstance.removeAddressFromBook(accounts[1], 1, 0x11, 2, {from: accounts[2]})
        .then(function(remvoeAdd){
            return ABinstance.getInfoOf(accounts[1], 1, 0x11);
        }).then(assert.fail).catch(function(error){
            assert(error.message.indexOf('revert') >= 0, "revert cause deleted Addr");
        });
    });

    it("register Org and then place account[5] in the addressBook of that org ",function(){
